...
import { Loader2, Zap, ArrowRight, Paperclip, Mic, Sparkle, X } from 'lucide-react';
    ...
    return (
      <div className="w-full max-w-4xl mx-auto px-4 pb-6 pt-2">
          ...
          >
            ...
            <div className="flex items-center gap-2 px-2 pt-1">
              <input
                ref={inputRef}
                type="text"
                placeholder={selectedFile ? `Ready to process: ${selectedFile.name}` : "Summarize YouTube video or upload a file..."}
                value={url}
                onChange={e => { setUrl(e.target.value); setError(''); setSelectedFile(null); }}
                className={`flex-1 bg-transparent border-none outline-none text-sm py-2 px-2 placeholder:text-white/20 focus:ring-0 ${selectedFile ? 'text-primary font-bold' : 'text-white'}`}
                disabled={isLoading}
              />
              {selectedFile && (
                <Button 
                  ...
                  variant="ghost" 
                  size="icon" 
                  onClick={() => { setSelectedFile(null); setUrl(''); }}
            ...
            <div className="flex items-center justify-between px-2 pb-1 border-t border-white/5 pt-2">
              <div className="flex items-center gap-1">
                <Button 
                  ...
                  className={`h-8 px-2 text-[10px] font-black gap-1.5 uppercase tracking-tighter transition-all ${selectedFile ? 'text-primary bg-primary/10' : 'text-white/30 hover:text-white hover:bg-white/5'}`}
                >
                  <Paperclip className="w-3.5 h-3.5" />
                  {selectedFile ? 'File Selected' : 'Attach Video'}
                </Button>
                <Button 
                  type="button"
                  variant="ghost" 
        ...
        <p className="mt-3 text-[9px] text-white/10 font-medium text-center uppercase tracking-[0.1em]">
          CortexOS — Neural Engine v3.0 // Tactical Research Terminal.
        </p>
      </div>
    );
  }
